...
type Props={children:ReactNode;onExport:()=>void;onReload:(exported:boolean)=>boolean|Promise<boolean>};
...
export default class LoadRecovery extends Component<Props,State>{
 ...
 render(){
  ...
  return <section className="load-recovery" role="alert" aria-label="Screen loading failed">
   ...
   <button type="button" onClick={async()=>{if(!await this.props.onReload(this.state.exported))this.setState({message:'Your work could not be saved on this device. Export a backup and check the download before reloading.'})}}>Reload latest version</button></div>
